...
c3<-ggplot(justcost, aes(y=CalendarYear)) +
  ...
  theme(plot.background = element_rect(fill='gray20', colour = NA), 
        ...
        legend.text = element_text(colour="gray98"),
        axis.ticks.x = element_line(colour='gray98')) +
  geom_point(aes(size = ControlAcres^(1/2), x = plot_date, color = cost, text=paste('Name:', FireName, '<br>Park:', ReportingUnitName, '<br>Type:', NEWCAT, '<br>Date:', format(StartDate, "%b %d %Y"), '<br>Duration:', duration, "days", '<br>WFU?:', ifelse(FireTypePr==14 | FireTypePr==49, "Yes", "No"), "<br>Size class:", SizeClass)), alpha=0.5) +
