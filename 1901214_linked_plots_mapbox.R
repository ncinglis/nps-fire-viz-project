...
print(object.size(fires), units="Mb")
...
fires_sfsimp<-st_simplify(fires, dTolerance = 100)



fires_mssimp<-ms_simplify(as(fires, "Spatial"), keep = 0.01) %>% st_as_sf()



print(object.size(fires_mssimp), units="Mb")

fires_mssimp
fires
 plot_mapbox(fires_mssimp)
...
plot_mapbox(share, color=~NEWCAT, colors=pal,
            ...
            text=~paste('Name:', FIRE_NAME, '<br>Park:', UnitName.x, '<br>Type:', NEWCAT, '<br>Date:', format(StartDate, "%b %d %Y"), '<br>Duration:', duration, "days", '<br>WFU?:', ifelse(FireTypePr==14 | FireTypePr==49, "Yes", "No"), "<br>Size:", ContrAcres), hoverinfo="text") %>% 
...
ggplotly(ggplot(share, aes(y=FIRE_YEAR)) +
           ...
           scale_y_reverse(limits = c(2017,1980), breaks = c(2015,2010,2005,2000,1980)) +
           xlab("") +
           ylab("") +
           ...
           geom_point(aes(size = ContrAcres*2, x = plot_date, color = NEWCAT, group=1, text=paste('Name:', FIRE_NAME, '<br>Park:', UnitName.x, '<br>Type:', NEWCAT, '<br>Date:', format(StartDate, "%b %d %Y"), '<br>Duration:', duration, "days", '<br>WFU?:', ifelse(FireTypePr==14 | FireTypePr==49, "Yes", "No"), "<br>Size:", ContrAcres)), alpha=0.5) +
           scale_color_manual(values=pal), tooltip="text") %>%
...
p<-ggplot(n, aes(y=FIRE_YEAR)) +
  ...
  scale_size_area(max_size = 15, guide=FALSE) +
  scale_x_date(date_breaks = "months", date_labels = "%b") +
  scale_y_reverse(limits = c(2018,1971), breaks = c(2015,2010,2005,2000,1995,1990,1985,1980)) +
  xlab("") +
  ylab("") +
  theme(plot.background = element_rect(fill='gray20'), 
        panel.background = element_rect(fill='gray20'), 
  ...
  geom_point(aes(size = ContrAcres*2, x = plot_date, color = NEWCAT, group=FIRE_YEAR, text=paste('Name:', FIRE_NAME, '<br>Park:', UnitName.x, '<br>Type:', NEWCAT, '<br>Date:', format(StartDate, "%b %d %Y"), '<br>Duration:', duration, "days", '<br>WFU?:', ifelse(FireTypePr==14 | FireTypePr==49, "Yes", "No"), "<br>Size:", ContrAcres)), alpha=0.5) +
